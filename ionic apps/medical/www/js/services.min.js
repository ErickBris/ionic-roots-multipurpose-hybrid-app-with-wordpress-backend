...
.factory('User', ["$http", "$rootScope", "$location", "$localstorage", function($http, $rootScope, $location, $localstorage) {
  ...
  return {
    ...
    register: function(nonce, name, username, email, password){
      return $http.jsonp( encodeURI( api+'user/register/?nonce='+nonce+'&display_name='+name+'&username='+username+'&email='+email+'&user_pass='+password+'&notify=no&callback=JSON_CALLBACK' ) );
    },
